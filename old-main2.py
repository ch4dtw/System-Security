...
for i in range(DATA_AMOUNT):
    ...
    for nByte in range(BYTE_AMOUNT):
        ...
        maxCC = 0.0 # correlation coefficient 相關係數
        maxKey = 0
        ...
        for key in range(KEY_AMOUNT):
            ...
            h_sum_temp = h_sum[ indexOf_h ] + h_now
            h_sum[ indexOf_h ] = h_sum_temp
            h_square_sum_temp = h_square_sum[ indexOf_h ] + (h_now ** 2)
            h_square_sum[ indexOf_h ] = h_square_sum_temp

            Lxx = h_square_sum_temp - ( (h_sum_temp**2) / (i+1) )

            # print(i, nByte, key)

            for point in range ( TRACE_AMOUNT ):

                sigma_h_times_t_temp = sigma_h_times_t[ indexOf_hXt ] + ( h_now*t_now[point] )
                sigma_h_times_t[ indexOf_hXt ] = sigma_h_times_t_temp

                Lyy = t_square_sum[point] - ( (t_sum[point]) / (i+1) )

                cc_fractions = sigma_h_times_t_temp - ((h_sum_temp*t_sum[point]) / (i+1)) # 分子

    ...
    for nByte in range(0, 16):
        for key in range(KEY_AMOUNT):
            if getY(plainTextList[i], nByte, key) % 2 == 0:
                ...
                (traceList[i]['A'] - LSB0_Avg[key + KEY_AMOUNT*nByte]) / (LSB0_Count[key + KEY_AMOUNT*nByte] + 1) )
                LSB0_Count[key + KEY_AMOUNT*nByte] += 1
            else:
                LSB1_Avg[key + KEY_AMOUNT*nByte] += (
                (traceList[i]['A'] - LSB1_Avg[key + KEY_AMOUNT*nByte]) / (LSB1_Count[key + KEY_AMOUNT*nByte] + 1) )
